...
func NewLLRpcClientOld(link string) *LLRpcClientOld {
	this := new(LLRpcClientOld)
	this.link = link
...
//连接
func (this *LLRpcClientOld) Init() *LLRpcClientOld {
	var err error
	//连接队列
...
func (this *LLRpcClientOld) listen() {
	//声明队列
...
//发布
func (this *LLRpcClientOld) Call(Path string, data []byte, timeOut int64) (res []byte, err error) {
	if timeOut == 0 {
		timeOut = 60
	}
...
//等待任务结果
func (this *LLRpcClientOld) waitResult(key string, timeOut int64) ([]byte, bool) {
	//注册监听通道
	this.lock.Lock()

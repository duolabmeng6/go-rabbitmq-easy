...
func TestPushQPS(t *testing.T) {
	...
	RpcServer.Router("testqps", 100000, func(messages amqp.Delivery) ([]byte, bool) {
		successCount.Add(1)

		msg := string(messages.Body)
		//E调试输出(msg)
		return E到字节集(msg + " ok"), false
	})

	//RpcServer2 := NewLLRpcServer("amqp://guest:guest@127.0.0.1:5672/")
	//RpcServer2.Router("testqps", 10000, func(messages amqp.Delivery) ([]byte, bool) {
	//	successCount.Add(1)
	//
	//	msg := string(messages.Body)
	//	//E调试输出(msg)
...
func TestNewLLRpcServer(t *testing.T) {
	...
	RpcServer.Router("func1", 10000, func(messages amqp.Delivery) ([]byte, bool) {
		successCount.Add(1)
